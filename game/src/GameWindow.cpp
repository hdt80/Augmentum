...
#include "game/components/UnitStatsComponent.h"
...
GameWindow::GameWindow(Vector2 size) {
	...
	if (!font.loadFromFile("res/Pixel.ttf")) {
		AG_WARN("Couldn't load \'%s\'", "Pixel.ttf");
	}
...
void GameWindow::keyEvent(sf::Event& e) {
	...
	if (e.key.code == sf::Keyboard::N) {
		AG_INFO("[GameWindow %x] Toggling on box2d shapeBit", this);
		// TODO: Dear god no
		...
		AG_INFO("b2DebugDraw flags: %d", Game::b2DebugDrawer.GetFlags());
